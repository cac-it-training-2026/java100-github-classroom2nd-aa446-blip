...
class Robot {
	...
	void makeOmeley(int eggNum, int butterNum) {
		int omeletegg = eggNum / 2;
		int omeletbutter = butterNum / 5;

		if (omeletbutter > omeletegg) {
			System.out.println("\n" + omeletegg + "人分のオムレツを作成しました。");

		} else {
			System.out.println("\n" + omeletbutter + "人分のオムレツを作成しました。");
	...
	String makeEggdishes(int flourNum, int sugarNum, int eggNum, int butterNum) {
		String menu = null;
		if (flourNum >= 170 && sugarNum >= 50 && energy >= 1 && butterNum >= 80) {
			menu = "クッキー";
